...
var addInventory = function(){
	...
	prompt.get(addInvt, function(err, res){
		...
		connection.query("UPDATE Products SET StockQuantity = (StockQuantity + ?) WHERE ItemID = ?;", 
			[inventoryUpdate[0].inventoryAmount, inventoryUpdate[0].inventoryID], function(err, result){
...
var addNewProduct = function(){
	...
	prompt.get(newProduct, function(err, res){
		...
		//connects to mysql and inserts response into the database 

		connection.query('INSERT INTO Products (ItemID, ProductName, DepartmentName, Price, StockQuantity) VALUES (?, ?, ?, ?, ?);', 
